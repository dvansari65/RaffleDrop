            ...
            {links.map(({ label, path }) => {
              ...
              return (
                ...
                >
                  {/* Hover / Active Glow */}
                  <div
                    className={`absolute inset-0 rounded-lg transition-all duration-300 ${active
                  ...
                  {/* Label */}
                  <span
                    className={`relative z-10 text-sm font-semibold tracking-wide transition-colors ${active ? "text-white" : "text-slate-300 group-hover:text-white"
                      }`}
                  >
                    {label}
                  </span>
                  ...
                  {/* Energy Underline */}
                  <span
                    className={`absolute left-1/2 -bottom-1 h-[2px] w-0 -translate-x-1/2 bg-gradient-to-r from-amber-400 to-yellow-300 rounded-full transition-all duration-300 ${active ? "w-8" : "group-hover:w-6"
                      }`}
                  />
            ...
            {links.map(({ label, path }) => {
              ...
              return (
                <Link
                  ...
                  className={`relative flex items-center justify-between px-4 py-3 rounded-xl transition-all ${active
