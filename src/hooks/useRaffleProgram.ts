...
export const useRaffleProgram = () => {
  ...
  const provider = useMemo(() => {
    ...
    const walletToUse = wallet.wallet ? wallet : new ReadOnlyWallet(PublicKey.default)

    return new AnchorProvider(connection, walletToUse as any, { commitment: 'confirmed' })
  }, [connection, wallet])

  const program = useMemo(() => {
    return new Program<Raffle>(idl as Raffle, provider)
  }, [provider])
